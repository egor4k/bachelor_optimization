...
StDialog::StDialog(Function*_F,Area _A,double _alpha,int _steps,QWidget *parent) :
    QDialog(parent),
    ui(new Ui::StDialog),alpha(_alpha),steps(_steps)
{
    F=_F;
    l.resize(2);
    r.resize(2);
...
void StDialog::on_pushButton_clicked()
{
    p=ui->prob->value();
    delta=ui->delt->value();
    Area A(l,r);
    x0.resize(2);
    Criteria*C=new Steps;
    std::list<std::vector<double>>tr;
    Stochastic*opt=new Stochastic;
    opt->set_p(p);
    opt->set_delta(delta);
    double eps=0;
    ...
    ui->k->setText(QString::number(steps));
...
void StDialog::mousePressEvent(QMouseEvent *event)
{
